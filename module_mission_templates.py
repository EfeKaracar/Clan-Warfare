...
0, 0, 0, [(eq, "$advanced_ai_open", 1),], [
   ...
   (try_for_agents, ":agents"),
        ...
        #SOURCE CHECK
		(neq, ":agents", ":player"),    # Agent is not player
		(agent_is_active, ":agents"),   # Agent is present
		(agent_is_alive, ":agents"),    # Agent is alive
		(agent_is_human, ":agents"),    # Agent is not horse
		(agent_slot_eq, ":agents", slot_agent_is_running_away, 0), # Agent is not fleeing
		
		(agent_get_troop_id, ":troop_no", ":agents"),
		(troop_get_slot, ":is_AI", ":troop_no", player_knows_how_to_play), #if agent has vyrn AI
		(eq, ":is_AI", 1), #yes
		(troop_get_slot, ":chance_AI", ":troop_no", player_skill_level),
	 
		(agent_get_team, ":agent_team", ":agents"),  # Get source team
	 
		#(assign, ":max_distance", 300), #distance between agents
		
		(agent_ai_get_look_target,":target", ":agents"),
		(gt,":target",0), #there is at least someone
		...
		(agent_get_animation, ":agent_anim", ":agents", 1),
		(agent_get_animation, ":pos_anim", ":target", 1),
		...
		(try_begin),
			...
			(eq, ":dice_for_avoid", 1),
				(try_begin),
					(eq, ":chance", 1),
					(agent_set_defend_action, ":agents", -2, 0),
					(agent_set_animation, ":agents", "anim_avoid_right_start"),
				(else_try),
					(eq, ":chance", 2),
					(agent_set_defend_action, ":agents", -2, 0),
					(agent_set_animation, ":agents", "anim_avoid_left_start"),
				(try_end),
		(try_end),
		#AI kick
		(try_begin),
			...
			(agent_set_animation, ":agents", "anim_vyrn_AI_kick"),
			(agent_get_bone_position, pos5, ":agents", hb_foot_r, 1),
			...
			(agent_deliver_damage_to_agent, ":agents", ":possible", ":rndm_damage"),#3 damage done atm.
