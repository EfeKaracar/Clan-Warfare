...
try_for_agents          =   12  # (try_for_agents, <destination>, [<position_no>], [<radius_fixed_point>]), #avoid using pos0
                                # Runs a cycle, iterating all agents on the scene. Iteration works for active agents alive and dead. Don't use check agent_is_active. It will always return true.
try_for_prop_instances  =   16  # (try_for_prop_instances, <destination>, [<object_id>], [<object_type>]), 
                                # Version 1.161+. Runs a cycle, iterating all scene prop instances on the scene. If <object_id> = <scene_prop_id> and <object_type> is not given, it loops through all instances. For object types see list of "somt_" in module_constants.py 
...
store_script_param          =   23  # (store_script_param, <destination>, <script_param_index>),
                                    # Retrieve the value of arbitrary script parameter (generally used when script accepts more than two). Parameters are enumerated starting from 1. Maximum are 15 parameters (16 operands maximum - 1 operand with script name).
...
key_clicked                     = 71  # (key_clicked, <key_code>),
                                      # Checks if the specified key is clicked in the current frame and wasn't clicked in the previous frame. See header_triggers.py for key code reference. Will work reliably only on mission template triggers that happen on every frame (i.e. check interval 0) as well as on simple_triggers or old form triggers that are check interval 0 too.
game_key_is_down                = 72  # (game_key_is_down, <game_key_code>),
                                      # Checks if the specified key is clicked in the current frame, ignoring if it was clicked in the previous frame. See header_triggers.py for game key code reference.
game_key_clicked                = 73  # (game_key_clicked, <game_key_code>),
                                      # Checks if the specified key is clicked in the current frame and wasn't clicked in the previous frame. See header_triggers.py for game key code reference. Will work reliably only on mission template triggers that happen on every frame (i.e. check interval 0) as well as on simple_triggers or old form triggers that are check interval 0 too.
...
party_set_extra_icon                  = 1682  # (party_set_extra_icon, <party_id>, <map_icon_id>, <vertical_offset_fixed_point>, <up_down_frequency_fixed_point>, <rotate_frequency_fixed_point>, <fade_in_out_frequency_fixed_point>),
                                              ...
                                              # Adds or removes an extra map icon to a party, possibly with some animations. Use (party_set_extra_icon, <party_id>, 0, 0, 0, 0, 0), - (0 as map_icon_id) - to remove extra icon.
...
troop_raise_proficiency                  = 1522  # (troop_raise_proficiency, <troop_id>, <proficiency_no>, <value>),
                                                 # Increases troop weapon proficiency by the specified value. Increase is subject to limits defined by Weapon Master skill. When used on non-hero troop, will affect all instances of that troop. Value can be negative(?)
...
cur_item_add_mesh                   = 1964  # (cur_item_add_mesh, <mesh_name_string>, [<lod_begin>], [<lod_end>]),
                                            # Version 1.161+. Only call inside ti_on_init_item trigger. Adds another mesh to item, allowing the creation of combined items. Note that in Native the operation (or more precisely the trigger it works in) is for helmets and armor only (it works for all items in inventory but it doesn't get called in missions). Parameter <mesh_name_string> should contain mesh name itself, NOT a mesh reference. LOD values are optional. If <lod_end> is used, it will not be loaded.
cur_item_set_material               = 1978  # (cur_item_set_material, <string_no>, <sub_mesh_no>, [<lod_begin>], [<lod_end>]),
                                            # Version 1.161+. Only call inside ti_on_init_item trigger. Replaces material that will be used to render the item mesh. Works only for main meta-mesh, so not for scabbards, etc. Use 0 for <sub_mesh_no> to replace material for base mesh. LOD values are optional. If <lod_end> is used, it will not be loaded.
...
race_completed_by_player              = 1004  # (race_completed_by_player),
                                              # Not documented. Not used in Native. Deprecated. Doesn't do anything, only return true.
...
is_zoom_disabled                      = 2222  # (is_zoom_disabled),
                                              # Version 1.153+. Checks that the zoom is currently disabled in the module.ini parameter disable_zoom = 1.
...
set_postfx                                   = 2386  # (set_postfx, <postfx_params-ID>)
                                                     # Not documented. Sets appearantly the respective postfx effect for the current scene. Count through your post_fx-settings beginning from 0 to get the ID for the effect or put the line 'ID_postfx_params import *' at the top to use directly the ID-name. Not used in Native.
...
scene_prop_set_visibility                   = 1813  # (scene_prop_set_visibility, <scene_prop_id>, <value>),
                                                    # Shows (value = 1) or hides (value = 0) the scene prop instance. The collision mesh remains, so you need to always run a seperate operation (prop_instance_enable_physics) to turn on and off the collision to match.
...
agent_is_in_special_mode                 = 1693  # (agent_is_in_special_mode, <agent_id>),
                                                 # Checks that the agent is currently in scripted mode. It's set after executing agent_set_scripted_destination or agent_set_scripted_destination_no_attack and is cleared with agent_clear_scripted_mode.
...
agent_is_alive                           = 1702  # (agent_is_alive, <agent_id>),
                                                 # Checks that the agent is alive. Only succeeds if agent is not down.
agent_is_wounded                         = 1703  # (agent_is_wounded, <agent_id>),
                                                 # Checks that the agent has been knocked unconscious. Fails for dead agents.
...
agent_is_non_player                      = 1707  # (agent_is_non_player, <agent_id>),
                                                 # Checks that the agent is not a player. Working in MP?
...
agent_is_active                          = 1712  # (agent_is_active, <agent_id>),
                                                 # Checks that the agent reference is active. This will succeed for dead or routed agents, for as long as the agent reference itself is valid. Engine will delete agent reference aproximatly 30 sec after his death.
...
agent_set_position                       = 1711  # (agent_set_position, <agent_id>, <position>),
                                                 # Teleports the agent to specified position on the scene, sets to ground level automatically though. Be careful with riders - you must teleport the horse, not the rider for the operation to work correctly! Gets synchronized by game engine automatically if called on server. Each time you call this operation the game sends a network event to all clients. Too many calls will fresh or mess up the client completely, so the only solution is not calling them 20 times per frame.
